...
function vipbp_handle_avatar_upload( $_, $file, $upload_dir_filter ) {
	...
	if ( 'bp_members_avatar_upload_dir' === $upload_dir_filter ) {
		update_user_meta(
			(int) $object_id,
			"vipbp-{$avatar_type}",
			array(
	...
	if ( BP_Attachment_Avatar::is_too_small( $bp->avatar_admin->image->file ) ) {
		bp_core_add_message(
			sprintf(
				__( 'You have selected an image that is smaller than recommended. For best results, upload a picture larger than %1$d x %2$d pixels.', 'buddypress' ),
...
function vip_handle_cover_image_upload( $_, $args, $needs_reset, $object_data ) {
	...
	// Upload file.
	$uploaded_file = isset( $_FILES['file'] ) ? $_FILES['file'] : '';
	$result        = wp_handle_upload(
		$uploaded_file,
		array(
			'action'    => 'wp_handle_upload', // Matches Core's action for uploads, to ensure VIP Go fileservice filters are applied.
